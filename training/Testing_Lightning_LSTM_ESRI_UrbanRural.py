...
if __name__ == "__main__":   
    ...
    if int(fold_order[0]) == 1:
        current_ckpt_base = "/mimer/NOBACKUP/groups/globalpoverty1/albin_and_albin/scripts_and_notebooks/job_scripts/lightning_logs/deeplabv3_esri_urban_rural_12345/checkpoints/12345_epoch=19-step=126080-val_loss=0.440.ckpt"
        current_ckpt_lstm = "/mimer/NOBACKUP/groups/globalpoverty1/albin_and_albin/scripts_and_notebooks/job_scripts/lightning_logs/deeplabv3_LSTM_esri_urban_rural_12345/checkpoints/12345_epoch=2-step=38304-val_loss=0.461.ckpt"
    elif int(fold_order[0]) == 3:
        current_ckpt_base = "/mimer/NOBACKUP/groups/globalpoverty1/albin_and_albin/scripts_and_notebooks/job_scripts/lightning_logs/deeplabv3_esri_urban_rural_34512/checkpoints/34512_epoch=19-step=106240-val_loss=0.369.ckpt"
        current_ckpt_lstm = "/mimer/NOBACKUP/groups/globalpoverty1/albin_and_albin/scripts_and_notebooks/job_scripts/lightning_logs/deeplabv3_LSTM_esri_urban_rural_34512/checkpoints/34512_epoch=2-step=32400-val_loss=0.402.ckpt"
    else:
        raise Exception("Model selected does not exist")

    
    lightning_model = DeepLabV3_LSTM_Lightning_ESRI_UrbanRural.load_from_checkpoint(current_ckpt_lstm, current_ckpt=current_ckpt_base, training_folds=training_folds, validation_fold=val_fold, fold_config=fold_order)
    
    
    
    # Step 3: Create Trainer --> Start testing
